...
def test():
    try:
        if(not 'time=' in (subprocess.check_output("ping 8.8.8.8 -c 4 -W 30 | grep 'time='", shell=True))):
...
if(sys.argv[1] == 'check'):
    if (apMode[0] is '1'):
        ...
        try:
            if('time=' in (subprocess.check_output("ping 8.8.8.8 -c 4 -W 30 | grep 'time='", shell=True))):
                print('Internet good')
        except subprocess.CalledProcessError as e:
